...
**Result.** Hard refusals on identical requests rise from 17% (bare) and 31% (app frame) to 58% (benchmark frame). Seven requests are answered in the app frame but withheld in the benchmark frame; none go the other way. The model's self-reported "is this an eval" score is flat across frames, so self-report is not a usable measure. Full write-up in `sleeper_circuits/reports/eval_frame_sensitivity.md`.

## Layout
Everything lives in `sleeper_circuits/`.
```
generate_dataset.py    asks an open-weight model on OpenRouter for borderline requests -> data/requests.json (751)
...
python3 generate_dataset.py --model nousresearch/hermes-4-405b --per-category 25 --rounds 2
